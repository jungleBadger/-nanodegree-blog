...
class Datastore:
    ...
    def create_entity(self, kind, value):
        if (kind == 'Post'):
            return datastore.Entity(self.datastore_client.key(kind, value),
                                    exclude_from_indexes=['image', 'text'])
        else:
    ...
    def delete_object(self, kind, value):
        return self.datastore_client.delete(
            self.datastore_client.key(kind, value))